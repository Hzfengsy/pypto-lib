...
def sparse_attn(
    ...
    for rp_idx in pl.spmd((H // 4) * 2, name_hint="rope"):
        ...
        for rp_hl in pl.range(0, 4):
            ...
            rp_o0 = rp_g * T + rp_t0
            # This head's full ROPE_DIM rope rows for this token tile (stride H); the
            # rotate math is FP32, so cast the strided BF16 slice to FP32 first.
            r_tile = pl.cast(
                pl.reshape(attn_rope_stage_3d[rp_t0 : rp_t0 + ROPE_OUT_TOK_TILE, rp_gh : rp_gh + 1, 0 : ROPE_DIM], [ROPE_OUT_TOK_TILE, ROPE_DIM]),
                target_type=pl.FP32)
